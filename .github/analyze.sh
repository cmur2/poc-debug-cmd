...
PR_NUMBER="$1"
PR_HEAD_SHA=$(gh api -X GET "repos/camunda/camunda/pulls/$PR_NUMBER" --jq '.head.sha')

echo "# Results for PR $PR_NUMBER"
echo ""

gh api -X GET "repos/camunda/camunda/actions/runs?sha=$PR_HEAD_SHA" --jq '.workflow_runs[]' | while read -r workflow_run; do
 ...
 workflow_run_attempt=$(echo "$workflow_run" | jq -r '.run_attempt')
  ...
  gh api -X GET "repos/camunda/camunda/actions/runs/$workflow_run_id/attempts/$workflow_run_attempt/jobs?per_page=100" --jq '.jobs[] | select(.conclusion=="failure") .id' | while read -r job_id; do
    >&2 echo "Checking failed job $job_id for well known failure reasons..."

    runner_problem_annotations=$(gh api -X GET "/repos/camunda/camunda/check-runs/$job_id/annotations" --jq '.[] | select(.message | contains("lost communication with the server") or contains("runner has received a shutdown signal"))')
    if echo "$runner_problem_annotations" | grep -q message; then
      ...
      echo "* [Job $job_id](https://github.com/camunda/camunda/actions/runs/$workflow_run_id/job/$job_id?pr=$PR_NUMBER) got aborted due to problems with runner \`$runner_name\`:"
    ...
    timeout_annotations=$(gh api -X GET "/repos/camunda/camunda/check-runs/$job_id/annotations" --jq '.[] | select(.message | contains("has exceeded the maximum execution time"))')
    if echo "$timeout_annotations" | grep -q message; then
      runner_name=$(gh api "repos/camunda/camunda/actions/jobs/$job_id" --jq '.runner_name' | tr '[:upper:]' '[:lower:]')

      >&2 echo "Job $job_id got cancelled due to timeout."

      echo "* [Job $job_id](https://github.com/camunda/camunda/actions/runs/$workflow_run_id/job/$job_id?pr=$PR_NUMBER) got cancelled due to timeout:"
